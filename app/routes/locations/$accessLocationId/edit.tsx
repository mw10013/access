...
import type { AccessLocation } from "@prisma/client";
import { db } from "~/utils/db.server";

type LoaderData = { accessLocation: AccessLocation };
...
export const action: ActionFunction = async ({
  ...
  if (request.method === "DELETE") {
    await db.accessLocation.delete({
      where: { id: Number(accessLocationId) },
    });
...
*/
  ...
  await db.accessLocation.update({
    where: { id: Number(accessLocationId) },
    data: { name, description },
  });

  return redirect(`/locations/${accessLocationId}`);
};

export default function Edit() {
  const { accessLocation } = useLoaderData<LoaderData>();
